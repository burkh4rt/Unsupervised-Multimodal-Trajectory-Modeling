...
class StateSpaceHybrid(ssm.StateSpaceModel):
    ...
    def __init__(
        self, *, n_neighbors: int | list = 10, n_folds: int = 3, alpha: float = 0.0
    ):
    ...
    def score(self, data: tuple[np.ndarray, np.ndarray]):
        ...
        for t in range(T - 1):
            ...
            idx_fin = np.isfinite(np.column_stack([states0, states1])).all(axis=1)
            log_likelihoods[idx_fin] += sp_stats.multivariate_normal(
                cov=self.state_model["covar"], allow_singular=True
            ).logpdf(states1[idx_fin] - states0[idx_fin] @ self.state_model["coeff"])
        for t in range(T):
            states0, meas0 = states[t], measurements[t]
            idx_fin = np.isfinite(np.column_stack([states0, meas0])).all(axis=1)
            log_likelihoods[idx_fin] += sp_stats.multivariate_normal(
